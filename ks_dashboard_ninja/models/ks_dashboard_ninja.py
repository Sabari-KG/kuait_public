...
class KsDashboardNinjaBoard(models.Model):
    ...
    def fetch_dashboard_overview(self, **kwargs):
        dashboards = self.env['ks_dashboard_ninja.board'].search_read(domain=[], fields=['name', 'is_bookmarked', 'ks_dn_dashboard_image'])
        bookmarked_dashboards = self.env['ks_dashboard_ninja.board'].search_count([('is_bookmarked', '=', True)])
        chart_count = 0
        map_count = self.env['ks_dashboard_ninja.item'].search_count([('ks_dashboard_item_type', '=', 'ks_map_view')])
        dashboards_info = {}
        ...
        for dashboard in dashboards:
            charts = self.env['ks_dashboard_ninja.item'].search_count(
        ...
        return {
            'overviewInfo': [len(dashboards), chart_count, map_count, bookmarked_dashboards, active_users],
    ...
    def update_bookmarks(self, *args):
        ...
        return self.env['ks_dashboard_ninja.board'].search_count([('is_bookmarked', '=', True)])
